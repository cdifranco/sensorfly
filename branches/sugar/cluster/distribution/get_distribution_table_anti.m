function distribution_table = get_distribution_table_anti(base_num, radius, max_dist, rang_dist_itvl, noise_rate)
...
real_radius_point_num = (max_dist - radius)/real_radius_itvl+1;
...
for i = 1:real_radius_point_num
    for j = 1:real_angle_point_num
        current_coord = [center_coord(1) + (radius + real_radius_itvl*(i-1))*cos(2*pi*j/real_angle_itvl), center_coord(2) + (radius + real_radius_itvl*(i-1))*sin(2*pi*j/real_angle_itvl)];
        for l = 1:base_num
...
item_num = 0;
for i = 1:real_radius_point_num
    real_dist = radius + real_radius_itvl*(i-1);
    for j = 1:real_angle_point_num
        for k = 1:sig_num_per_point
...
for i = 1:size(distribution_table, 1)
    distribution_table(i, 4) = 1 - distribution_table(i, 3)/max_cnt;
end

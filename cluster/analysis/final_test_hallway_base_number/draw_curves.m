...
figure;
%}
errorbar(4:2:12, dist_error_collection_controlled(p2,1), dist_error_collection_controlled(p2,2),'b');
%hold on;
%errorbar(4:2:12, dist_error_collection_random(p2,1), dist_error_collection_random(p2,2),'r');
%hold off;
figure;
errorbar(4:2:12, step_collection_controlled(p2,1), step_collection_controlled(p2,2),'b');
hold on;
errorbar(4:2:12, step_collection_random(p2,1), step_collection_random(p2,2),'r');
hold off;
figure;
plot(4:2:12, step_collection_controlled(p2,1), 'b');
hold on;
plot(4:2:12, step_collection_random(p2,1), 'r');
hold on;
plot([4,12], [1/0.45, 1/0.45], 'g');
hold off;
axis ([4, 12, 0, max(step_collection_random(p2,1))]);

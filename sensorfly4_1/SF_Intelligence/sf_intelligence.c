...
/*! \fn void sf_get_position_task_init
    \brief  Initializes the get position task
    \return none
*/
...
/*! \fn uint32_t request_distance(uint8_t dest)
    \brief  Sends a range request and waits for the response, resends requests on timeout
    \params dest the mac address of the node to send the ranging request to
    \return uint32_t the distance from the current node to the destination in millimeters
*/
...
/*! \fn void sf_swarm_system_start()
    \brief  Initial setup of the mapping system, listens for data packets with the ranges between OX and OY
    \params none
    \return none
*/
void sf_swarm_system_start()
{
...
/*! \fn void task_get_position_func(void * par)
    \brief  sends ranging requests to all reference node and plots current position based on response
    \params * par a pointer, for future use
    \return none
*/
void task_get_position_func(void * par)
{
   // sf_swarm_system_start();
    ...
    while(1){
        ...
        //calculate the second set of coordinates based on node X and the origin node
        ...
        //take the average of the two sets of coordinates
        xCoodSet1=(xCoodSet1+xCoodSet2)/2;
        yCoodSet1=(yCoodSet1+yCoodSet2)/2;
        

        //convert to the proper position in the coordinate grid
        if(yNeg==1){
          yCoodSet1=-yCoodSet1;
        }
        if(xNeg==1){
...
//simple broadcast function for the fixed point nodes needs to be worked on to contain all initial procedures for the fixed reference points
...
//simple broadcasts function for the fixed reference point that acts as the origin, go to range listen mode and wait
